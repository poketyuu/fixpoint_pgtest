...
public:
    ...
    string GetSubnetString(){
        bitset<32> subnetmask(0);
        subnetmask = (~subnetmask) << (32 - prefix);
        bitset<32> networkaddress = subnetmask & ipv4address;
        bitset<32> b(255);
        long long l1 = ((networkaddress & (b << 24)) >> 24).to_ullong();
        long long l2 = ((networkaddress & (b << 16)) >> 16).to_ullong();
        long long l3 = ((networkaddress & (b << 8)) >> 8).to_ullong();
        long long l4 = (networkaddress & b).to_ullong();
        return to_string(l1) + "." + to_string(l2) + "." + to_string(l3) + "." + to_string(l4) + "/" + to_string(prefix);
    }
...
bool ChangeSubnetState(map<Address,bool> &subnetmap,Address &ad,bool state){
    auto subnetfind = subnetmap.find(ad);
    if (subnetfind != subnetmap.end())
    {
        subnetfind->second = state;
    }
    bool alltimeout = true;
    auto iter = subnetmap.begin();
    while (iter != subnetmap.end())
    {
        alltimeout = alltimeout && iter->second;
        ++iter;
    }
    return alltimeout;
}
int main(int args, char *argv[])
{
    ...
    map<long long, map<Address, bool>> subnetmap;
    map<string, AccessLog> subnetmonitor;
    string line;
    while (getline(ifs, line))
    {
        ...
        auto subnetresult = subnetmap.insert(make_pair(ad.subnet().to_ullong(), (map<Address,bool>){}));
        auto subnetmapresult = subnetresult.first->second.insert(make_pair(ad, false));
        ...
        if (!result.second)
        {
            ...
            else
            {
                if (alog.success)
                {
                    ...
                    prevlog = alog;
                    map<Address, bool> &subnetmap = subnetresult.first->second;
                    if (!ChangeSubnetState(subnetmap, ad, false))
                    {
                        auto subnetfind = subnetmonitor.find(ad.GetSubnetString());
                        if(subnetfind != subnetmonitor.end()){
                            AccessLog &startlog = subnetfind->second;
                            time_t prev = mktime(&startlog.accesstime);
                            time_t cur = mktime(&alog.accesstime);
                            double timeout = difftime(cur, prev) + alog.ping / 1000.0;
                            subnetlog.push_back(Errlog(subnetfind->first, prevlog.OutputAccesstime(false), alog.OutputAccesstime(true), timeout));
                            subnetmonitor.erase(subnetfind->first);
                        }
                    }
                }
                else
                {
                    ...
                    if(prevlog.misscount = N){
                        map<Address, bool> &subnetmap = subnetresult.first->second;
                        if(ChangeSubnetState(subnetmap,ad,true)){
                            subnetmonitor.insert(make_pair(ad.GetSubnetString(), prevlog));
                        }
                    }
                }
            }
    ...
    {
        subnetofs << subnetlog[i].ServerAddress << "," << (subnetlog[i].start + "-" + subnetlog[i].end) << "," << subnetlog[i].TimeoutTerm << endl;
    }
    subnetofs.close();
    return 0;
}